...
echo "Updating Kubernetes ConfigMaps with latest configurations..."

# First, create a backup of the current ConfigMap
echo "Creating backup of current ConfigMap..."
kubectl get configmap -n saml-tools -o yaml > k8s/configmaps-backup-$(date +%Y%m%d-%H%M%S).yaml

# Create a new ConfigMap YAML with updated configurations
cat > k8s/01-configmaps-https-updated.yaml << 'EOF'
apiVersion: v1
kind: ConfigMap
metadata:
  name: samlidp-config
...
kubectl apply -f k8s/01-configmaps-https-updated.yaml
