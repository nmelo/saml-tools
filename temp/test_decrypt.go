...
func main() {
	// Create a dummy example to illustrate the API
...
func decryptAssertion(response *saml.Response, privateKey *rsa.PrivateKey) (*saml.Assertion, error) {
    ...
    if err := doc.ReadFromBytes(plaintextBytes); err \!= nil {
        return nil, fmt.Errorf("failed to parse decrypted assertion: %v", err)
    }
    
    // Create a new assertion
    assertion := &saml.Assertion{}
    if err := saml.Unmarshal(doc.Root(), assertion); err \!= nil {
